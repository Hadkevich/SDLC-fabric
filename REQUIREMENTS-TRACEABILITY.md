...
| S2.b | *Stretch:* self-improving / feedback loop | 🟡 | Bounded fix loop for **both** a rejected review **and a failed e2e run** (`_request_rework`/`_drain_rework`/`_apply_rework`, `engine.py:578–631`; per-stage cap `STAGE_REWORK_CAP` — review 2, e2e 1) + minimal post-deploy `_monitor` fold to `backlog.json`. Full live-telemetry re-planning is future work (`SPEC.md §3.9`) |
