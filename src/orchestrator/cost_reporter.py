...
def build_cost_report(project_root: Path, repo_root: Optional[Path] = None) -> dict:
    ...
    totals = {"cost_usd": 0.0, "input_tokens": 0, "output_tokens": 0,
              "total_tokens": 0, "duration_ms": 0.0, "event_count": 0}
...
def render_markdown(report: dict) -> str:
    ...
    for role, r in report["by_agent_role"].items():
        lines.append(
            ...
            f"{r['total_cost_usd']:.3f} | {r['total_duration_ms'] / 1000:.0f}s | {r['coverage']} |"
        )
    lines.append(
        f"| **TOTAL** | | {t['event_count']} | {t['input_tokens']:,} | "
        f"{t['output_tokens']:,} | {t['cost_usd']:.3f} | {t['duration_ms'] / 1000:.0f}s | |"
    )
    if report.get("notes"):
        lines += ["", "**Notes**"]
