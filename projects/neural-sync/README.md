...
> 243/243 tests pass (77 at the original recorded run), the review gate caught one contract defect (**BLK-001**) which was
...
The current QA suite: **243/243 passing, 0 failing** (77 at the original recorded run;
the suite has since grown to 243 across the operator-profile, team-mismatch, skill-alias,
profile-enrichment, data-ingestion/ANN, and admin-override work — run it with
`docker exec neural-sync-backend-1 pytest`), all 13 acceptance criteria covered —
including the good-match (score ≥ 0.75) and
